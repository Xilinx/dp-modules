...
/******************************************************************************/
/**
 * This function prints the stream information to passed buffer
 *
 * @param	Stream is a pointer to video stream.
 ...
 *
*******************************************************************************/
int XVidC_ShowStreamInfo(const XVidC_VideoStream *Stream, char *buff, int buff_size)
{
	int strSize = 0;
	...
	if (!XVidC_GetVideoModeData(Stream->VmId) &&
			(Stream->VmId != XVIDC_VM_CUSTOM)) {
		strSize += scnprintf(buff+strSize, buff_size-strSize,
			"\tThe stream ID (%d) is not supported.\r\n",
			Stream->VmId);
		return strSize;
	}
	...
	strSize += scnprintf(buff+strSize, buff_size-strSize,
		"\r\n\tHSYNC Timing: hav=%04d, hfp=%02d, hsw=%02d(hsp=%d), " \
		"hbp=%03d, htot=%04d \n\r", Stream->Timing.HActive,
			Stream->Timing.HFrontPorch, Stream->Timing.HSyncWidth,
			Stream->Timing.HSyncPolarity,
			Stream->Timing.HBackPorch, Stream->Timing.HTotal);

	/* Interlaced */
	if (Stream->IsInterlaced) {
	strSize += scnprintf(buff+strSize, buff_size-strSize,
		"\tVSYNC Timing (Field 0): vav=%04d, vfp=%02d, " \
		"vsw=%02d(vsp=%d), vbp=%03d, vtot=%04d\n\r",
			Stream->Timing.VActive,       Stream->Timing.F0PVFrontPorch,
			Stream->Timing.F0PVSyncWidth, Stream->Timing.VSyncPolarity,
			Stream->Timing.F0PVBackPorch, Stream->Timing.F0PVTotal);
	strSize += scnprintf(buff+strSize, buff_size-strSize,
		"\tVSYNC Timing (Field 1): vav=%04d, vfp=%02d, " \
		"vsw=%02d(vsp=%d), vbp=%03d, vtot=%04d\r\n",
			Stream->Timing.VActive,      Stream->Timing.F1VFrontPorch,
			Stream->Timing.F1VSyncWidth, Stream->Timing.VSyncPolarity,
			Stream->Timing.F1VBackPorch, Stream->Timing.F1VTotal);
	}
	/* Progressive */
	else {
	strSize += scnprintf(buff+strSize, buff_size-strSize,
		"\tVSYNC Timing: vav=%04d, vfp=%02d, " \
		"vsw=%02d(vsp=%d), vbp=%03d, vtot=%04d\r\n\r\n",
			Stream->Timing.VActive,       Stream->Timing.F0PVFrontPorch,
			Stream->Timing.F0PVSyncWidth, Stream->Timing.VSyncPolarity,
			Stream->Timing.F0PVBackPorch, Stream->Timing.F0PVTotal);
