...
static inline int idt_write_reg(struct idts *priv, u16 addr, u8 val)
{
	...
	if (err)
		printk("i2c write failed, addr = %x\n", addr);
...
static int IDT_8T49N24x_Configure_JA(u32 I2CBaseAddress, u8 I2CSlaveAddress)
{
	...
	for (index = 8 ; index < sizeof(IDT_8T49N24x_Config_JA) ; index++) {
		...
		msleep_range(10);
		if (index != 0x070) {
			ret = idt_write_reg(idt, index, IDT_8T49N24x_Config_JA[index]);
		}
	}
	return ret;
}
...
/* The configuration is started from address 0x08 */
	for (index=8; index<sizeof(IDT_8T49N24x_Config_Syn); index++) {
		...
		if (index != 0x070) {
			ret = idt_write_reg(idt, index,IDT_8T49N24x_Config_Syn[index]);
			msleep_range(10);
		}
	}
	return ret;
...
int IDT_8T49N24x_Init(void)
{
	...
	if(ret)
		printk("IDT_8T49N24x_Enable 0  I2C progmming failed\n");
#if 1 
	msleep_range(30);
	/* Configure device. */
	ret = IDT_8T49N24x_Configure_JA(XPAR_IIC_0_BASEADDR, I2C_IDT8N49_ADDR);
	if(ret)
                printk("IDT_8T49N24x_Configure_JA I2C progmming failed\n");

	msleep_range(30);
	/* Enable DPLL and APLL calibration. */
	ret = idt_enable(TRUE);
	if(ret)
                printk("IDT_8T49N24x_Enable 1 I2C progmming failed\n");
#endif
	msleep_range(30);
	return 0;
}
