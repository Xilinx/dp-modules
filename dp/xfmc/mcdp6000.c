...
static inline int mcdp6000_read_reg(struct mcdp6000 *priv, u16 addr, u32 *val)
{
	...
	if (err)
		printk("mcdp6000 i2c read failed, at addr 0x%x\n", addr);
	return err;
}
...
static inline int mcdp6000_write_reg(struct mcdp6000 *priv, u16 addr, u32 val)
{
	...
	if (err)
		printk("mcdp6000 i2c write failed, at addr 0x%x\n", addr);
	return err;
}
			...
			const struct i2c_device_id *id)
{
	...
	/* initialize regmap */
	mcdp6000->regmap = devm_regmap_init_i2c(client, &mcdp6000_regmap_config);
